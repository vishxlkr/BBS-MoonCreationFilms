  ...
  useEffect(() => {
    const ctx = gsap.context(() => {
      ...
      if (headlineRef.current) {
        tl.fromTo(headlineRef.current, { y: 80, opacity: 0 }, { y: 0, opacity: 1, duration: 1.2, ease: "power3.out" });
      }
      if (sublineRef.current) {
        tl.fromTo(sublineRef.current, { y: 40, opacity: 0 }, { y: 0, opacity: 1, duration: 0.9, ease: "power3.out" }, "-=0.6");
      }
      if (ctaRef.current) {
        tl.fromTo(ctaRef.current, { y: 30, opacity: 0 }, { y: 0, opacity: 1, duration: 0.7, ease: "power3.out" }, "-=0.5");
      }
      if (scrollDotRef.current) {
        tl.fromTo(scrollDotRef.current, { opacity: 0 }, { opacity: 1, duration: 0.5, ease: "power3.out" }, "-=0.3");
      }
    });
  ...
  return (
    <section className="relative w-full h-[100vh] overflow-hidden">
      ...
      <div className="relative z-20 flex flex-col items-center justify-center h-full text-center px-6 mt-12">
        ...
        <div ref={sublineRef}>
          <p 
            className="font-body text-base sm:text-lg md:text-2xl text-ice-blue max-w-2xl mb-10 tracking-wide font-light"
          >
            Visual storytelling crafted for the moments that matter. We don&apos;t just capture video; we create films.
          </p>
        </div>
        <div ref={ctaRef}>
          <Link 
            ...
            data-cursor="view"
          >
            <span className="relative z-10 group-hover:text-charcoal-night">View Our Work</span>
      ...
      <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-20 flex flex-col items-center gap-2">
        <div ref={scrollDotRef} className="w-[6px] h-[6px] bg-ice-blue rounded-full animate-bounce" />
      </div>
    </section>
  );
}
