...
export default function Lightbox({ isOpen, onClose, videoUrl, title, description, onNext, onPrev }: LightboxProps) {
  useEffect(() => {
  ...
  const content = (
    <AnimatePresence>
      <div className="fixed inset-0 z-[200] flex items-center justify-center p-4 sm:p-8">
        ...
        <button
          onClick={onClose}
          className="absolute top-6 right-6 z-[210] text-white hover:text-cinematic-blue transition-colors p-2 bg-black/20 rounded-full"
        >
          <X size={28} />
        </button>
        ...
        >
          ...
          {(title || description) && (
            <div className="text-center">
              {title && <h2 className="font-heading text-2xl md:text-3xl text-ice-blue mb-2">{title}</h2>}
              {description && <p className="font-body text-sm text-ice-blue/70 max-w-2xl mx-auto">{description}</p>}
            </div>
          )}
        </motion.div>
      </div>
