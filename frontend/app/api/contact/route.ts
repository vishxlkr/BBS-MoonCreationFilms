...
export async function POST(req: Request) {
  try {
    ...
    const { fullName, phone, email, service, eventDate, message, referral } = validatedData;
    ...
    const toEmail = process.env.RESEND_TO_EMAIL || contactData.email;
    ...
    if (error instanceof z.ZodError) {
      return NextResponse.json({ error: "Validation failed: " + (error as any).errors[0].message }, { status: 400 });
    }
